...
func (m *Message) SendFile(c interface{}, file string, fw *fsworker.Fsworker, owner uint) (bytesSent int, err error) {
...
func (m *Message) ProcessFullAuth(c *net.Conn, db *gorm.DB, tokenValidDays int) (uid uint, newToken string, err error) {
	// Parse payload
	auth, err := m.ValidateAuth()
	if err != nil {
		return uid, newToken, fmt.Errorf("[ValidateAuth] error validating -> %w", err)
	}
	// Check credentials
	ok, uid, err := users.ValidateCreds(auth.Login, auth.Password, db)
	if err != nil {
		return uid, newToken, fmt.Errorf("[ValidateCreds] error validating -> %w", err)
	}
	if !ok {
		return uid, newToken, fmt.Errorf("wrong credentials")
	}
	// Token for the client
	newToken, err = users.GenerateToken()
	if err != nil {
		return uid, newToken, fmt.Errorf("[GenerateToken] error generating -> %w", err)
	}
	// Add token into DB
	err = users.RegisterToken(uid, newToken, db, tokenValidDays)
	if err != nil {
		return uid, newToken, fmt.Errorf("[RegisterToken] error registering -> %w", err)
	}
